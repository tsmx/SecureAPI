...
module.exports.registerUser = function (req, res) {
    ...
    newUser.save((s_err, s_res) => {
        if (s_err) {
            console.log('registerUser: error while creating new user: ' + s_err.message);
            res.status(403).json({ error: 'cannot create user' });
        }
        else {
            console.log('registerUser: new user created: ' + s_res.username);
            res.status(200).json({
                message: 'user created',
                userName: s_res.username,
                validationCode: s_res.activation.key
            });
        }
    });
};
...
module.exports.activateUser = function (req, res) {
    ...
    userSchema.findOne({ username: req.query.username }, (error, user) => {
        if (error) {
        ...
        user.save((s_err, s_res) => {
            if (s_err) {
                console.log('activateUser: activation could not be saved in db: ' + s_err.message);
                res.sendStatus(403);
                return;
            }
            else {
